    ...
    function addComplete(success, detectedPortBlocking)
    {
        if (!success) {
            ...
            if (detectedPortBlocking) {
                errorDialog.text += "\n\n" + qsTr("This PC's Internet connection is blocking PartyZone. Streaming over the Internet may not work while connected to this network.")
            }
            else {
        ...
        onClicked: {
            if (model.online) {
                if (!model.serverSupported) {
                    errorDialog.text = qsTr("The version of GeForce Experience on %1 is not supported by this build of PartyZone. You must update Moonlight to stream from %1.").arg(model.name)
                    errorDialog.helpText = ""
                    errorDialog.open()
                }
