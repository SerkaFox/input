...
#include "vigem/vigemwidget.h"

extern VigemWidget * vigem_widget;
...
void SdlInputHandler::performSpecialKeyCombo(KeyCombo combo)
{
    ...
    case KeyComboActivateVigem:
        qDebug() << "activating Vigem";
        if (vigem_widget) {
            vigem_widget->callVigemStartup();
        }

        break;

    case KeyComboDeactivateVigem:
        qDebug() << "deactivating Vigem";
        if (vigem_widget) {
            vigem_widget->callVigemShutdown();
        }
